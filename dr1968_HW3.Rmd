...
#For this model, I plan to use the logistic regression algorithm. Since I am building a model with a categorical variable with only two classes, logistic regression makes the most sense. Linear regression requires that the response variable be continuous. However, since I am working with a categorical response variable, linear regression is not possible. In addition, logistic regression predicts the probabilities of an event occurring. In this case, the model being built will predict whether a tumor is benign or malignant.

library(mlbench)
data("BreastCancer")

#Must first omit rows that contain NA values
BreastCancer <- na.omit(BreastCancer)

#Next, Must remove ID column as that will not serve as a predictor for the model
...
#All variables are factors. Only Class should be a factor while others are set to numeric
...
#Now, all predictors are numeric values rather than factors


table(BreastCancer$Class)

#There are more benign than malignant samples. The data must be evenly split

Benign<-BreastCancer[which(BreastCancer[,"Class"] == "benign"),]
Malignant<-BreastCancer[which(BreastCancer[,"Class"] == "malignant"),] 
sample(1:nrow(Benign), nrow(Malignant))->rand.0
Benign[rand.0,]->Benign
Malignant->IsMalignant
split_Benign <-sample(nrow(Benign), floor(nrow(Benign)*0.7))
split.train_Benign <-Benign[split_Benign,]
split.test_Benign <- Benign[-split_Benign,]
split_Malignant <- sample(nrow(IsMalignant),floor(nrow(Benign)*0.7))
split.train_Malignant <-IsMalignant[split_Malignant,]
split.test_Malignant <-IsMalignant[-split_Malignant,]
training_set_Cancer <-rbind(split.train_Malignant, split.train_Benign)
test_set_Cancer <-rbind(split.test_Malignant, split.test_Benign)
table(training_set_Cancer$Class)

#Now there is an even number of benign to malignant samples
...
ctrl <- trainControl(method = "repeatedcv", repeats = 15,classProbs = T,
                     savePredictions = T)
LogisticCancerModel <- train(Class ~., data=training_set_Cancer, method = "glm", family="binomial", trControl = ctrl)
##ctrl <- trainControl()
...
summary(LogisticCancerModel)

#After checking the model summary, now must predict probabilities on test data
Predict.cancer <- predict(LogisticCancerModel, newdata = test_set_Cancer)

#Produced the confusion matrix for the model on the test data
confusionMatrix(Predict.cancer, reference = test_set_Cancer$Class)

#Accuracy of 0.9306 reported. This model performs exceptionally well.
```
```{r}

#Visualize the ROC curve 
plot(x = roc(predictor = LogisticCancerModel$pred$benign,
             response = LogisticCancerModel$pred$obs)$specificities, 
     y = roc(predictor = LogisticCancerModel$pred$benign, 
             response = LogisticCancerModel$pred$obs)$sensitivities,
     col= "blue", xlim = c(1, 0), type ="l", ylab = "Sensitivity",
     xlab = "Specificity")
legend("bottomright", legend = paste("Benign v Malignant --", 
                                     roc(predictor = LogisticCancerModel$pred$benign,
                                         response = LogisticCancerModel$pred$obs)$auc
, sep = ""), col = c("blue"), fill = c("blue"))

#AUC value of 0.989217
```

```{r}
Predict.cancer <- predict(LogisticCancerModel, newdata = test_set_Cancer, type = "prob")
#Log odds ratio
odds_class1 <- Predict.cancer[,1] / (1 - Predict.cancer[,1])
log_odds_class1 <- log(odds_class1)
cor.test(log_odds_class1, test_set_Cancer$Cl.thickness)
plot(log_odds_class1, test_set_Cancer$Cl.thickness)
```
